(use-package org
  ...
  (org-generic-id-locations-file
   (no-littering-expand-var-file-name "org/org-generic-id-locations"))
  ...
  :config (defun cfg-setup-org-tags
              ()
            "Setup org tags"
            (turn-on-auto-fill)
            (let* ((date-re "[0-9]\\{4\\}-[0-9]\\{2\\}-[0-9]\\{2\\}")
              ...
              (setq svg-tag-tags
                    `(;; Org tags
                      ...
                      ("\\(personal\\):" .
                       ((lambda (tag)
                          (svg-lib-tag "personal" nil :stroke 0 :font-weight 'semibold  :background "#a3be8c" :foreground "#2e3440"))))
                      ("\\(work\\):" .
                       ((lambda (tag)
                          (svg-lib-tag "work" nil :stroke 0 :font-weight 'semibold :background "#b48ead" :foreground "#2e3440"))))
  ...
  (let* ((calendar-dir (concat org-directory "calendars/"))
         ...
         (cmd
          (format "curl -s -o %s %s && %s -count -d %s -a %s %s"
                  ...
                  calendar-file calendar-file ics-file)))
...
(defun org-calendar-sync-all (callback)
  ...
  (let ((calendars
         '(("Personal calendar" "dmerenne@gmail.com" "https://calendar.google.com/calendar/ical/dmerenne%40gmail.com/private-c885f6b2cd33378cfef6e35733b91bd4/basic.ics" "personal")
           ("Work calendar" "damien@bloom-life.com" "https://calendar.google.com/calendar/ical/damien%40bloom-life.com/private-29f3b5faabf9628322f2663dacadb417/basic.ics" "work"))))
...
(defun org-calendar-wait (callback process event)
  ...
  (unless org-calendar-sync-processes (funcall callback)))
