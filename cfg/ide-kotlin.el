...
(use-package kotlin-mode
  ...
  (defun kotlin-open-in-intellij ()
    ...
    (interactive)
    (save-buffer)
    (start-process "intellij-launch" nil "intellij-idea-community"
                   "--line"
...
(defun test-runner-kotest-test-name-at-point ()
  ...
  (save-excursion
    (replace-regexp-in-string
     ...
     (replace-regexp-in-string
      "[[:blank:][:cntrl:]]+" " "
      (if (or (looking-at test-runner-kotest-string-spec-test-regex)
...
(defun test-runner-kotest-test-class-at-point ()
  ...
  (save-excursion
    (goto-char (point-min))
    (let ((package
           (when (looking-at test-runner-kotest-package-regex)
             (match-string 1)))
          (class
           (when (re-search-forward test-runner-kotest-class-regex nil t)
             (match-string 1))))
...
(defclass
 test-runner-backend-kotest
 (test-runner-backend-exec)
...
(cl-defmethod test-runner-backend-exec-arguments-test-at-point
    ((_backend test-runner-backend-kotest))
...
(cl-defmethod test-runner-backend-exec-environment-test-at-point
    ((_backend test-runner-backend-kotest))
  "Return the arguments to pass to binary to run test at point."
  (let ((test-class (test-runner-kotest-test-class-at-point))
        (test-name (test-runner-kotest-test-name-at-point)))
    (list
     (format "kotest_filter_specs=%s" test-class) (format "kotest_filter_tests=%s" test-name))))
...
(cl-defmethod test-runner-backend-exec-arguments-test-project
    ((_backend test-runner-backend-kotest))
  "Return the arguments to pass to binary to run test at point."
  (let ((test-project (test-runner-kotest-project))
        (test-subproject (test-runner-kotest-subproject)))
...
(test-runner-define-compilation-mode
 kotest
