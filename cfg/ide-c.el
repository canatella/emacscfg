...
(use-package cmake-mode
  :ensure t
  :custom (cmake-tab-width 4 "configure indent"))
...
(use-package eglot-clangd
  :quelpa (eglot-clangd :fetcher github :repo "canatella/eglot-clangd")
  :after (cc-mode eglot)
  :bind (:map c-mode-base-map ("C-c o" . eglot-clangd-switch-source-header)))

(use-package test-runner-catch2
  :quelpa (test-runner-catch2 :fetcher github :repo "canatella/test-runner-catch2-el")
  :after (test-runner))

(use-package cmake-api
  :quelpa (cmake-api :fetcher github :repo "canatella/cmake-api-el"))
...
(use-package cc-mode
  ...
  (c-doc-comment-style
   '((c-mode . doxygen)
     ...
     (pike-mode . autodoc)))
  :devdocs (c++-mode "cpp")
  :diminish ((c++-mode . "🅲++") (c++//lw-mode . "🅲++"))
  :mode (("\\.impl\\'" . c++-mode) ("\\.ino\\'" . c++-mode))
  :config
  (defun config-c-mode-flyspell-check-word-predicate ()
    "Used for `flyspell-generic-check-word-predicate' in c modes."
    (and (flyspell-generic-progmode-verify)
         (save-excursion
           (beginning-of-line)
           (not (looking-at "^// NOLINT")))))
...
(use-package djinni-mode
  :quelpa (djinni-mode :fetcher github :repo "canatella/djinni-mode"))
