...
(use-package
  ...
  :diminish
  :custom (ivy-magic-slash-non-match-action #'ivy-magic-slash-non-match-create
                                            "Create directory when hitting / without matches")
  (counsel-describe-function-function #'helpful-callable)
  (counsel-describe-variable-function #'helpful-variable)
  (ivy-sort-matches-functions-alist '((t . ivy--shorter-matches-first)
...
specify the read-command as the final optional arg to this function."
    ...
    (mapcar (lambda (file)
		    ...
		    (while do-recurse
                      (setq dir-files  (funcall read-directories-command (concat path file)
						;; all but . and ..
						nil "^[^\\.]\\{1,2\\}") file-count (length
                                                                                    dir-files))
                      (cond
			    ...
			    (when mohkale/ivy-collapse-paths--append-files
                       ...
                       (t
                        (setq do-recurse nil))))))) file) files))
      ...
      (cl-letf* (((symbol-function 'actual-directory-files)
                 ...
                 ((symbol-function 'read-file-name-internal)
                  (lambda
                    (&rest
                     args)
                    (collapse-directories default-directory (apply #'actual-read-file-name-internal
                                                                   args) #'actual-directory-files)))
                 ((symbol-function 'directory-files)
                  (lambda (dir)
                    (collapse-directories dir (mapcar
        ...
        (apply func dir nil))))
  (advice-add 'ivy--sorted-files
              :around #'counsel-find-file--collapse-directories-wrapper))
...
(use-package
  ...
  :bind (("C-x C-f" . counsel-find-file)
         ...
         ([f9] . counsel-ag))
  :config (let ((done (where-is-internal #'ivy-done     ivy-minibuffer-map t))
